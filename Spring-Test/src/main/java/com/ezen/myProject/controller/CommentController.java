...
public class CommentController {
	 ...
	 * produces: 보낼 때 형태{MediaType.test_plain_value}
	 */
	@PostMapping(value = "/post", consumes = "application/json", produces = {MediaType.TEXT_PLAIN_VALUE})
	public ResponseEntity<String> post(@RequestBody CommentVO cvo){
		log.info(">>> comment post : "+cvo.toString());
		int isUp = csv.register(cvo);
		log.info(">>> register isUp : "+(isUp>0?"성공":"실패"));
		return isUp>0? new ResponseEntity<String>("1",HttpStatus.OK)
				:new ResponseEntity<String>(HttpStatus.INTERNAL_SERVER_ERROR);
	}
	
	@GetMapping(value= "/{bno}", produces = {MediaType.APPLICATION_JSON_VALUE})
	public ResponseEntity<List<CommentVO>> spread(@PathVariable("bno")int bno){ //싣고 온 bno을 매개변수로 받기
		log.info(">>> comment list : "+bno);
		List<CommentVO> list = csv.getList(bno);
		return new ResponseEntity<List<CommentVO>>(list,HttpStatus.OK); //리스트라는 객체에 http상태를 보냄
	}
	...
	public ResponseEntity<String>edit(@PathVariable("cno")int cno,@RequestBody CommentVO cvo){
		...
		log.info(">>> comment modify bno: "+cvo.toString());
